...
export default function Admin() {
  ...
  const links = (s) => (s||'').split(',').map(x=>x.trim()).filter(Boolean).map((u,i)=>(<a key={i} className="link mr-2" href={u} target="_blank" rel="noreferrer">{u}</a>));
        ...
        <table className="table">
          <thead className="bg-slate-100">
            <tr>
              ...
              <th className="th">Medical Docs & Imaging URLs</th>
          ...
          <tbody className="divide-y divide-slate-200">
            {rows.map(r => (
              <tr key={r.id}>
                ...
                <td className="td">{links(r.documentsUrls)}</td>
                <td className="td">
                ...
                <td className="td">
                  <div className="flex flex-col gap-2">
                    ...
                    <button className="btn" onClick={()=>downloadPdf(r.id)}>Generate PDF</button>
